...
class Client implements \CharlotteDunois\Events\EventEmitterInterface {
     ...
     *   'ws.intents' => int, (which server intents to register to. defaults to all. OR together Client::WS_INTENTS)
    ...
    public function getAllIntents(): int
    {
        $intent = 0;
        foreach (self::WS_INTENTS as $val) {
            $intent |= $val;
        }
        return $intent;
    }
