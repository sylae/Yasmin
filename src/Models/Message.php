...
class Message extends ClientBase {
    ...
    function _patch(array $message)
    {
        ...
        $this->nonce = \CharlotteDunois\Yasmin\Utils\DataHelpers::typecastVariable(($message['nonce'] ?? null),
            'string');
        $this->pinned = (bool)($message['pinned'] ?? $this->pinned);
        $this->system = (isset($message['type']) ? (!in_array($message['type'], [0, 19, 20])) : $this->system);
        $this->type = (isset($message['type']) ? self::MESSAGE_TYPES[$message['type']] : $this->type);
        $this->webhookID = \CharlotteDunois\Yasmin\Utils\DataHelpers::typecastVariable(($message['webhook_id'] ?? $this->webhookID),
            'string');
        $this->activity = (!empty($message['activity']) ? (new \CharlotteDunois\Yasmin\Models\MessageActivity($this->client,
            $message['activity'])) : $this->activity);
        $this->application = (!empty($message['application']) ? (new \CharlotteDunois\Yasmin\Models\MessageApplication($this->client,
            $message['application'])) : $this->application);
