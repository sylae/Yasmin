...
class MessageReaction extends ClientBase {
     ...
     *
     * @param \CharlotteDunois\Yasmin\Models\User|string $user Defaults to all users.
     *
     * @return \React\Promise\ExtendedPromiseInterface
    ...
    function remove($user = null) {
        return (new \React\Promise\Promise(function (callable $resolve, callable $reject) use ($user) {
            if ($user !== null) {
                $this->client->apimanager()->endpoints->channel
                    ->deleteMessageUserReaction($this->message->channel->getId(), $this->message->id,
                        $this->emoji->identifier, $user->id)
                    ->done(function () use ($resolve) {
                        $resolve($this);
                    }, $reject);
            } else {
                $this->client->apimanager()->endpoints->channel
                    ->deleteMessageReactionsForEmoji($this->message->channel->getId(), $this->message->id,
