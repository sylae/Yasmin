...
class MessageCreate implements \CharlotteDunois\Yasmin\Interfaces\WSEventInterface
{
    ...
    function handle(\CharlotteDunois\Yasmin\WebSocket\WSConnection $ws, $data): void {
        ...
        if($channel instanceof \CharlotteDunois\Yasmin\Interfaces\TextChannelInterface) {
            ...
            $repMessage = null;
            if (array_key_exists('referenced_message', $data) && is_array($data['referenced_message'])) {
                $repChannel = $this->client->channels->get($data['referenced_message']['channel_id']);
                if ($repChannel instanceof \CharlotteDunois\Yasmin\Interfaces\TextChannelInterface) {
                    $repMessage = $repChannel->_createMessage($data['referenced_message']);
                }
            }
            ...
            if ($message->guild && $message->mentions->users->count() > 0 && $message->mentions->users->count() > $message->mentions->members->count()) {
                ...
                foreach ($message->mentions->users as $user) {
                    $promise[] = $message->guild->fetchMember($user->id)->then(function (
                        \CharlotteDunois\Yasmin\Models\GuildMember $member
                    ) use ($message) {
                ...
                if ($message->replyToID ?? false) {
                    $promise[] = $repChannel->fetchMessage($message->replyToID)->then(function (
                        \CharlotteDunois\Yasmin\Models\Message $repliedTo
                    ) use ($message) {
                        $message->replyTo = $repliedTo;
                    }, function () {
                        // Ignore failure
                    });
                }
